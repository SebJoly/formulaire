 ...
 * @author 
 */
class FormulaireDriverPGSQL extends FormulaireDriver {
	...
	public function isIndex($name) {
		...
		$requete['parameters'] = array("table" => strtolower($this->getFormulaire()->table), "constraint" => strtolower($this->getFormulaire()->table) . "_pkey", "column" => $name);
	...
	public function isAutoGenerated($name) {
		$requete['requete'] = 'SELECT TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=:table AND COLUMN_NAME = :column AND COLUMN_DEFAULT LIKE \'next_val(%)\';';
